...
class run_model:
    ...
    def do_eval(self,sess, data_set):
        ...
        for step in xrange(steps_per_epoch):
            ...
            loss_value, query_weights, doc_weights = sess.run([self.loss_op,self.query_weights, self.doc_weights],  feed_dict=feed_dict)
            #query_weights = np.asarray(query_weights)
            query_weights_all.append(query_weights)
            doc_weights_all.append(doc_weights)
            #doc_weights = np.asarray(doc_weights)
            total_loss += loss_value

            #query_weights = np.concatenate(query_weights)
            #doc_weights = np.concatenate(doc_weights)

        pickle.dump (query_weights_all, open(self.config.outdir + "query_weights.pkl", "w"))
        pickle.dump(doc_weights_all, open(self.config.outdir + "doc_weights.pkl", "w"))
        return float(total_loss)/float(steps_per_epoch)
    ...
    def run_training(self):
        ...
        with tf.Graph().as_default():
            ...
            self.logits, self.query_weights, self.doc_weights  = self.model.inference(self.encode_input_placeholder, self.decode_input_placeholder,
