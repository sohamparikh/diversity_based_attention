...
class run_model:
    ...
    def do_eval(self,sess, data_set):
        ...
        for step in xrange(steps_per_epoch): 
            ...
            loss_value, ctx_vec = sess.run([self.loss_op, self.ctx_vec], feed_dict=feed_dict)
            total_loss += loss_value


	    norm_ctx_vec = []
	    for  i in ctx_vec:
		norm_ctx_vec.append(np.average(i, axis = 0))

	    print (norm_ctx_vec)
	    final_ctx_vec.append(norm_ctx_vec)
            total_loss += loss_value


	x = np.array(final_ctx_vec)
	m = np.average(x, axis=0)
	print (m)
	pickle.dump(m , open(self.config.outdir + "saturation.pkl", "w"))
        return float(total_loss)/float(steps_per_epoch)
    ...
    def run_training(self):
        ...
        with tf.Graph().as_default():
            ...
            self.logits, self.ctx_vec = self.model.inference(self.encode_input_placeholder, self.decode_input_placeholder, 
