...
from basic_files.static_dr import *
...
class BasicAttention:
    ...
    def inference(self, encoder_inputs1, decoder_inputs1, query_inputs, embedding_size, feed_previous,
                  len_vocab, hidden_size, weights,  lengths_query, embedding_trainable,  initial_embedding = None, c=None):
        ...
        distract_cell = DistractionLSTMCell_hard(hidden_size, state_is_tuple = True)
        cell_encoder_bw = GRUCell(hidden_size)
        #enc_cell = DistractionLSTMCell(hidden_size)
        ei = tf.unpack(encoder_inputs1)
        #decoder_inputs1 = tf.transpose(decoder_inputs1)
        di = tf.unpack(decoder_inputs1)
        #qi = tf.unpack(query_inputs)

        outputs, state     = static_seq2seq(encoder_inputs = ei,
                                                decoder_inputs = di,
                                                ...
                                                sequence_lengths_query = lengths_query,
