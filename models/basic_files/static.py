...
def static_decoder(decoder_inputs,
                      initial_state,
  ...
  with variable_scope.variable_scope(
      scope or "static_decoder", dtype=dtype) as scope:
    dtype = scope.dtype
    ...
    if initial_state_attention:
      list_of_queries = [initial_state, query_embeddings]
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      if i == 0 and initial_state_attention:
        with variable_scope.variable_scope(variable_scope.get_variable_scope(),
          ...
          list_of_queries =  [state, query_embeddings]
          attns_state = attention(list_of_queries)
      else:
        list_of_queries = [state, query_embeddings]
        attns_state = attention(list_of_queries)
...
def static_decoder_wrapper(decoder_inputs,
                                initial_state,
                                distract_initial_state,
                                attention_states,
                                query_embeddings,
  ...
  with variable_scope.variable_scope(
    embedding_scope or "static_decoder_wrapper", dtype=dtype,  reuse = True) as s1:
  ...
  with variable_scope.variable_scope(
    scope or "static_decoder_wrapper", dtype =dtype) as scope:
...
def static_seq2seq(encoder_inputs,
                                decoder_inputs,
                                query_inputs,
                                cell_encoder_fw,
                                cell_encoder_bw,
  ...
  """
  with variable_scope.variable_scope(
      scope or "static_seq2seq", dtype=dtype) as scope:
    dtype = scope.dtype
    # Encoder.
    ...
    query_embeddings = array_ops.concat(1, [query_state_fw, query_state_bw])
    #query_embeddings = query_outputs[0]
    ...
    def decoder(feed_previous_bool):
      ...
      with variable_scope.variable_scope(
        ...
        outputs, state = static_decoder_wrapper(
            decoder_inputs,
            initial_state=encoder_state,
