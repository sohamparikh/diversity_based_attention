...
def dynamic_decoder(decoder_inputs,
                      initial_state,
  ...
  with variable_scope.variable_scope(
      scope or "dynamic_decoder", dtype=dtype) as scope:
    dtype = scope.dtype
    ...
    print (attn_length_state, attn_length_query, attn_size_state, attn_size_query)
    ...
    if initial_state_attention:
      attns_query, attention_weights  = attention_query(initial_state)
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      x = linear([inp] + attns_state, input_size, True)
      ...
      if i == 0 and initial_state_attention:
        with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                           reuse=True):
          attns_query, temp = attention_query(state)
          list_of_queries =  [state, attns_query]
          attns_state, temp1 = attention(list_of_queries)
      else:
        attns_query, temp = attention_query(state)
        list_of_queries = [state, attns_query]
        attns_state, temp1 = attention(list_of_queries)

      query_weights.append(temp)
      doc_weights.append(temp1)
  ...
  print ("Decoder" , len(outputs), len(query_weights), len(doc_weights))
  return outputs, state, query_weights, doc_weights

def dynamic_decoder_wrapper(decoder_inputs,
                                initial_state,
                                distract_initial_state,
                                attention_states,
                                attention_states_query,
  ...
  with variable_scope.variable_scope(
    embedding_scope or "dynamic_decoder_wrapper", dtype=dtype,  reuse = True) as s1:
  ...
  with variable_scope.variable_scope(
    scope or "dynamic_decoder_wrapper", dtype =dtype) as scope:
